...
vec3 raycast(int startC, vec3 startPos, vec3 startDir, float startDist)
{
    ...
    vec4 tint = vec4(0.);
    for (int i=0;i<int(maxDist * sqrt(3.))+3;i++) {
        float bestDist = 4.;
        ...
        if (next == -1 || dist + bestDist > maxDist)
        {
...
void main()
{
    //gl_FragColor = 40. * (texture2D(mapImg, (floor(vTexCoord.xy * mapSize) + .5) / mapSize));
    //gl_FragColor.a = 1. + sin(time) * -0.;
    //return;
    //initArray();
    float viewRad = tan(fov * asin(1.) / 180.);
    vec3 cam = normalize(vec3(viewRad * 2. * (vTexCoord.xy - .5), 1.));  // .5
    cam *= mat3(1., 0., 0., 
                0., cos(phi), sin(phi), 
                0., -sin(phi), cos(phi));
